 ...
 */
  ...
  */
 function edit_theme_comment_count() {
  ...
  */
 function edit_theme_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && edit_theme_categorized_blog() ) {
  ...
  * @return bool
  */
 function edit_theme_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'edit_theme_categories' ) ) ) {
 		...
 		set_transient( 'edit_theme_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so edit_theme_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so edit_theme_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in edit_theme_categorized_blog.
  */
 function edit_theme_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'edit_theme_categories' );
 }
 add_action( 'edit_category', 'edit_theme_category_transient_flusher' );
 add_action( 'save_post',     'edit_theme_category_transient_flusher' );
  ...
  */
 function edit_theme_comment_list( $comment, $args, $depth ) {
  ...
  */
 function edit_theme_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
